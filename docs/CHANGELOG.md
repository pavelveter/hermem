...
### Code review remediation (round on extraneous hardening)

Cross-cutting fixes from a code-review pass on the 0.3.x line; all isolated changes with dedicated test coverage where applicable.


- **fix(api): SIGHUP-driven OpenAPI cache invalidation** — `api/spec.go` dropped the legacy `sync.Once` + package-level `cachedSpec *Spec` pattern in favour of `atomic.Pointer[Spec]` + `specMu sync.Mutex` (double-checked lock around `Build()` to prevent a stampede-herd rebuild on first concurrent requests). Added `api.InvalidateSpec()` so the cache can be dropped without a process restart. `api/handler.go` is now stateless — both `handleJSON` and `handleYAML` call `GenerateSpec()` per-request — and serves with `Cache-Control: no-cache` so the HTTP layer cannot defeat SIGHUP invalidation through intermediary caches. `src/internal/cli/serve.go` wires `api.InvalidateSpec()` into the SIGHUP reload loop alongside `srv.ReloadState(...)` and `store.StoreSchemaFingerprint(...)`. The SIGHUP contract is documented as defensive / forward-compatible today: the spec is currently built from compile-time Go types (`AllSchemas()` / `AllPaths()`), so the invalidation lever has no semantic effect unless a future builder grows config-driven content (e.g. allowed_categories enum). `api/openapi_test.go` adds `TestInvalidateSpecAllowsBuildVersionFlip` (three explicit assertions locking the cache-vs-Invalidate contract; the middle step carries a LOAD-BEARING comment) and strengthens `TestSpecVersionDeterministic` with a `defer`-restore of `BuildVersion`.

- **fix(sdk): Content-Type guard in `(*Client).do`** — the error branch now skips `json.Unmarshal(respBody, &errResp)` when the response `Content-Type` does not advertise `application/json`, falling through to the raw-body `APIError` path that already existed in the codebase. A 502 from an upstream proxy with `Content-Type: text/html` now correctly surfaces the raw HTML in `APIError.Message` instead of attempting-then-failing JSON unmarshal. `sdk/go/client_test.go` adds four regression tests: text/plain-with-sniffing (close proxy of "no Content-Type"), 502 + `text/html`, 500 + `application/json` with an invalid JSON body (fallback to raw), and 400 + structured `ErrorResponse` envelope (happy path).

- **fix(plugin/memory/hermem): deterministic memory IDs via SHA-256** — `hermem_store` now derives its `id` from a 16-hex-char prefix of `hashlib.sha256(content.encode("utf-8")).hexdigest()` instead of `hash(content) & 0xFFFFFFFF:08x`. Python's built-in `hash()` is per-process salted by `PYTHONHASHSEED`, which meant the same content produced different ids across agent sessions and forced hermem's b2 dedup-merge runner to consume LLM embedding calls even for memory that was already stored. Plugin version bumped 0.4.0 → 0.5.0 — this is an ID-format break: any entity written before this commit (id format `mem-<8hex>`) will NOT dedup-merge with new entities (`mem-<16hex>`) on the same content. Operators with pre-fix databases should expect old rows to remain frozen-unique; new writes use the deterministic format.

- **feat(ci): `go mod tidy` guard** — new top-level `modtidy` job fails the build if `go mod tidy` would mutate `go.mod` / `go.sum`. Catches the failure mode where a PR adds an import or transitive dep without regenerating the lockfile. The job sits at the same workflow-default `permissions: contents: read` level as `test` / `lint` / `govulncheck` and runs `scripts/ensure-embed-placeholders.sh` before `go mod tidy` for parity with every other compile-touching job.


- **chore(gitignore): explicit `.venv/` patterns** — added `.venv/`, `**/.venv/`, `venv/`, `**/venv/` to the root `.gitignore` as defense-in-depth. The nested `sdk/python/.venv/.gitignore` already existed (Python tooling auto-stamps it when creating a virtual environment), but the explicit root-level rules close any path-dependency holes.
