...
The `install-mcp.sh` script configures hermem MCP in all detected AI tools:

```bash
# Interactive — toggle which tools to configure
./scripts/install-mcp.sh

# Non-interactive — configure everything detected
./scripts/install-mcp.sh --all --yes

# Specific tools only
./scripts/install-mcp.sh --tools=claude_desktop,cursor,opencode

# Preview changes without writing
./scripts/install-mcp.sh --all --dry-run

# Remove hermem from all tool configs
./scripts/install-mcp.sh --uninstall
```

Supported tools: `claude_desktop`, `claude_code`, `codex`, `cursor`, `opencode`, `aider`, `hermes`.
...
`~/.config/opencode/opencode.json` — top-level `"mcp"` key:
...
Hermes manages MCP via its own CLI, not config file editing:

```bash
hermes mcp add hermem --command hermem --args mcp
```

Verify: `hermes mcp list`

Remove: `hermes mcp remove hermem`
...
| `memory_retrieve` | Retrieve contextual memories around given seed IDs |
