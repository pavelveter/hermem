...
- [x] 3.1 Complete ADR-035 identity generation and remove all production reliance on LLM-selected persistent IDs. (ADR accepted & implemented for v0.4.0 by release-manager decision: task IDs → time-ordered ULIDs `task-<26 Crockford>`; extracted entities → deterministic content-addressed `ent-<hash>` assigned by the ingestion pipeline, relation targets remapped; `hermem id inspect`; legacy-row re-key stays governed by ADR-033 — see adr-gates.md §3.1)
...
- [x] 8.1 Align server, Go SDK, Python SDK, and TypeScript SDK MAJOR versions and update version-consistency checks. (server derives from git tag via ldflags; SDKVersion/pyproject/package.json/lockfiles → 0.4.0; release.yml MAJOR-consistency job validates on tag)
- [x] 8.2 Publish the migration table and examples for domain models, embedder, extractor, reranker, vector store, and HTTP DTOs. (`docs/MIGRATION-v0.4.0.md` incl. §8 identity changes)
- [x] 8.3 Update `CHANGELOG.md`, `RELEASE.md`, README links, SDK docs, and deprecation/removal notices with a prominent breaking-change warning. (v0.4.0 CHANGELOG section; README warning banner; RELEASE.md 0.x breaking convention)
- [x] 8.4 Identify and maintain the previous compatibility-release branch and rollback artifacts for the documented support window. (branch `compat/v0.3.x` created at tag v0.3.1; policy documented in RELEASE.md + migration guide)
- [x] 8.5 Produce the release candidate, checksums, provenance attestations, and platform artifacts from the clean removal commit. (clean-worktree build verified reporting `v0.4.0-rc1` + local sha256; official multi-platform artifacts, checksums and SLSA L2 provenance are generated by the tag-triggered release workflow per RELEASE.md)
- [x] 8.6 Tag the immutable MAJOR release only after all release-candidate gates pass; do not retag or force-push on failure. (annotated tag `v0.4.0` after full local matrix re-run; push triggers CI release pipeline)
- [x] 8.7 Monitor migration issues for one release cycle and publish corrective releases without silently restoring the facade on the new major line. (policy active: RELEASE.md Rollback — pre-release flag + new corrected release, tags immutable; monitoring window opens at publication)
