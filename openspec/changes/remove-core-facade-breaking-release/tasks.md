...
> Remaining in these packages: constructor-level capability interfaces
> (`core.VectorIndex`/`Embedder`/`LLMExtractor`/`Reranker`) and transport
> request DTOs (`StoreRequest`/`EdgeRequest`) — tracked by 4.5/4.6 and the
> 6.x deletion tasks.
...
- [ ] 4.3 Migrate ingestion, memory, edge, re-embedding, health, retention, and task services away from core types. (value types done: domain alias sweep + id/retention/migration ownership moves complete; remaining: `VectorIndex`/`Embedder`/`LLMExtractor` constructor params)
