...
- `verify_test.go`'s `TestTiming` now exercises three cohorts (1k / 3k / 6k entities) from a single shared file-backed DB and prints `PERF  N=<n>  search=<dur>  retrieve=<dur>` lines per cohort. Hand-typed `~2.3ms` numbers in README `## Performance` are replaced by the test's actual output (grep-friendly, reproducible on any machine via `go test -v -run TestTiming -count=1 ./...`). The strict < 5ms regression gate stays at N=1000 only; larger-N rows are informational.
