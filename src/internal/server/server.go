// Package server provides the HTTP API server with all handlers and middleware.
package server
...
func (s *Server) HandleTaskCreate(w http.ResponseWriter, r *http.Request) {
	...
	state := s.State.Load()
	cat := FirstStatefulCategory(state.Schema)
	if cat == "" {
...
// DecodeStrict parses JSON while rejecting unknown fields.
...
func FirstStatefulCategory(schema core.SchemaConfig) string {
	keys := make([]string, 0, len(schema.StatefulCategories))
	for k := range schema.StatefulCategories {
		keys = append(keys, k)
	}
	if len(keys) == 0 {
		return ""
