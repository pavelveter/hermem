...
func InitMetricsDB(db *sql.DB) {
	db.Exec(`CREATE TABLE IF NOT EXISTS metrics_entity_access (entity_id TEXT NOT NULL, accessed_at DATETIME DEFAULT CURRENT_TIMESTAMP)`) //nolint:errcheck // async metrics-worker flush
	db.Exec(`CREATE INDEX IF NOT EXISTS idx_metrics_access ON metrics_entity_access(entity_id, accessed_at)`)                             //nolint:errcheck // async metrics-worker flush
}
