...
// EvidenceLister fetches all evidence records for a belief.
// evidence.Service satisfies this interface.
type EvidenceLister interface {
	ListForBelief(ctx context.Context, beliefID int64) ([]EvidenceItem, error)
}
...
func PropagateConfidence(ctx context.Context, bSvc belief.Service, eLst EvidenceLister, beliefID int64) (float64, error) {
	if beliefID <= 0 {
		return 0, fmt.Errorf("evolution: invalid belief ID %d", beliefID)
	}

	all, err := eLst.ListForBelief(ctx, beliefID)
	if err != nil {
	...
	for _, e := range all {
		...
		case "support":
			supportSum += e.GetStrength()
		case "refute":
			refuteSum += e.GetStrength()
		}
	}
